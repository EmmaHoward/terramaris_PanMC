...
from file_split_2km import packing
# system paths
...
units = {
  ...
  "m01s30i462":"kg/m/s",
  "m01s30i463":"kg/m/s",
  "mass_fraction_of_rain_in_air":"kg/kg"
}


def postprocess_output(date1,outname,stream):
    path = "/work/scratch-nopw/emmah/pepf/%04d%02d%02dT0000Z/sep1/"%(date1.year,date1.month,date1.day)
...
#      print(date2)
      times = [date2+ dt.timedelta(j/24) for j in np.arange(24)]
#      times[0] = times[0].replace(minute=0)
      if outname in ["cloud_fraction","mixing_ratios","radar_reflectivity","density","northward_wind","pressure_rho_grid","single_level","upward_air_velocity","eastward_wind","potential_temperature","pressure_theta_grid","specific_humidity"]:
        files = [path+"%04d%02d%02d%02d_%s_%s.nc"%(t.year,t.month,t.day,t.hour,outname,stream) for t in times]
      else:
        files = [path+"%04d%02d%02d%02d_%s_%s.nc"%(t.year,t.month,t.day,t.hour,"single_level",stream) for t in times]
      print(files)
...
def copy_ncks_remove(date1,outname,stream):
    # postprocess data from crun starting on date1 into file outname.nc
    cell_methods,variables = file_variables[outname] 
    # loop through times for file output
    for i in range(2,6):
      date2 = date1 + dt.timedelta(i)
      outfile = "tma_2km_KPPcoupled_%s_%s_%04d%02d%02d.nc"%(region[stream],outname,date2.year,date2.month,date2.day)
      print(outfile)
...
copy_ncks_remove(t1,var,stream)
